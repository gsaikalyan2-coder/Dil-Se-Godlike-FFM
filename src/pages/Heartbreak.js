      ...
      <main className="flex-1 relative z-10 pt-20">
        ...
        <section className="relative min-h-screen flex flex-col items-center justify-center text-center px-6 pt-16 overflow-hidden">
          ...
          <div className="relative z-20 max-w-4xl mx-auto flex flex-col items-center justify-center">
             ...
             {/* ═══ Interactive Candle Widget (Glassmorphic) ═══ */}
             <div className={`mt-16 transition-all duration-[1.5s] ease-out delay-1000 ${mounted ? 'opacity-100' : 'opacity-0'}`}>
                <button 
                  ...
                  className={`group relative flex flex-col items-center gap-3 px-12 py-8 bg-[#13110e]/60 backdrop-blur-xl border transition-all duration-500 rounded-3xl overflow-hidden hover:-translate-y-2 shadow-2xl hover:shadow-[0_20px_60px_rgba(230,195,100,0.15)] focus:outline-none ${
                    candles > 0 ? 'border-[#e6c364]/60 bg-[#1a1710]/90' : 'border-white/10 hover:border-[#e6c364]/40'
                  }`}
                >
                  {/* Subtle Background Inner Glow */}
                  {candles > 0 && <div className="absolute inset-0 bg-gradient-to-b from-[#e6c364]/10 to-transparent pointer-events-none" />}
                  
                  <span 
                    className={`material-symbols-outlined text-5xl transition-all duration-300 relative z-10 ${
                      candles > 0 ? "text-[#e6c364] animate-flicker" : "text-white/30 group-hover:text-[#e6c364]/60"
                    }`} 
                    style={{'fontVariationSettings': candles > 0 ? "'FILL' 1" : "'FILL' 0"}}
                  >
                    {candles > 0 ? "local_fire_department" : "workspace_premium"}
                  </span>
                  
                  <div className="text-center relative z-10">
                    <span className={`block font-headline text-sm font-bold uppercase tracking-[0.3em] transition-colors ${candles > 0 ? 'text-[#e6c364]' : 'text-white/60 group-hover:text-white'}`}>
                      {candles > 0 ? 'Keep The Fire Alive' : 'Light a Candle'}
                    </span>
                    <span className={`block font-body text-xs mt-2 font-medium tracking-wider uppercase transition-colors ${candles > 0 ? 'text-white/80' : 'text-white/30'}`}>
        ...
        <section className="py-24 px-6 border-t border-white/5 relative z-20 bg-[#050505]/95">
          <div className="max-w-4xl mx-auto text-center">
             ...
             <div className="flex flex-col sm:flex-row flex-wrap justify-center gap-6">
                ...
                {/* Links created in prior revisions */}
                <a 
